...
public class ChromeDownloadDelegate
    ...
    public void requestHttpGetDownload(DownloadInfo downloadInfo) {
        MediaDownloadManager mediaDownloadManager = new MediaDownloadManager(mContext, this);
    ...
    private boolean launchInfoBarIfFileExists(final DownloadInfo info) {
        boolean sweDownloadSelectionEnabled = CommandLine.getInstance().
                                   hasSwitch(SWEBrowserSwitches.DOWNLOAD_PATH_SELECTION);
        mDownloadPath = null;
        // Checks if file exists.
        final String fileName = info.getFileName();
        File dir = null;
        if (sweDownloadSelectionEnabled)
            dir = Environment.getExternalStoragePublicDirectory(info.getPreferredFolderForMime());
        else
        ...
        if (!sweDownloadSelectionEnabled && !file.exists()) {
            return false;
        }
    ...
    private boolean enqueueDownloadManagerRequestFromNative(
        ...
        if (mDownloadPath != null) {
            DownloadInfo newDownloadInfo = DownloadInfo.Builder.fromDownloadInfo(downloadInfo)
                .setFilePath(mDownloadPath).build();
            mDownloadPath = null;
            return enqueueDownloadManagerRequestInternal(newDownloadInfo);
        } else {
    ...
    public void updateDownloadPath(String downloadpath) {
        ArrayList<InfoBar> currentInfoBars = mTab.getInfoBarContainer().getInfoBars();
        for(InfoBar infoBar: currentInfoBars) {
            if (infoBar instanceof DownloadInfoBar) {
                ((DownloadInfoBar) infoBar).updateDownloadPath(downloadpath);
